<?php
        ...
        $reqserie = "SELECT name FROM series WHERE name LIKE :serie;";
        $reqserie=$connection->prepare($reqserie);

        $querysaison = "SELECT seasons.number FROM seasons INNER JOIN seriesseasons ON seasons.id = seriesseasons.season_id INNER JOIN series ON seriesseasons.series_id=series.id WHERE series.name LIKE :serie AND seasons.number=:numsaison;";
        $reqsaison=$connection->prepare($querysaison);

        $queryepisode="SELECT episodes.number FROM episodes INNER JOIN seasonsepisodes ON episodes.id=seasonsepisodes.episode_id INNER JOIN seasons ON seasons.id=seasonsepisodes.season_id INNER JOIN seriesseasons ON seasonsepisodes.season_id=seriesseasons.season_id INNER JOIN series ON seriesseasons.series_id=series.id WHERE series.name LIKE :serie AND seasons.number=:numsaison AND episodes.number=:numepisode;";
